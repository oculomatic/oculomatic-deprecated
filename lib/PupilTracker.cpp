...
public:
    HaarSurroundFeature(int r1, int r2) : r_inner(r1), r_outer(r2)
    {
        ...
        //val_inner = std::sqrt( (float)count_outer/(count_inner*count_outer + sq(count_inner)) );
        //val_outer = -std::sqrt( (float)count_inner/(count_inner*count_outer + sq(count_outer)) );
    ...
    )
{
    ...
    SECTION("Grey and crop", log)
    {
        ...
        {
          throw std::runtime_error("Oops! We only support mono image...");
        }
    }
    ...
    SECTION("Haar responses", log)
    {
        ...
        for (int r = params.Radius_Min; r < params.Radius_Max; r+=rstep)
        {
            ...
            // Use TBB for rows
            std::pair<float,cv::Point2f> minRadiusResponse = tbb::parallel_reduce(
                tbb::blocked_range<int>(0, (mEye.rows-r - r - 1)/ystep + 1, ((mEye.rows-r - r - 1)/ystep + 1) / 8),
                std::make_pair(std::numeric_limits<float>::infinity(), UNKNOWN_POSITION),
                [&] (tbb::blocked_range<int> range, const std::pair<float,cv::Point2f>& minValIn) -> std::pair<float,cv::Point2f>
            {
                std::pair<float,cv::Point2f> minValOut = minValIn;
                for (int i = range.begin(), y = r + range.begin()*ystep; i < range.end(); i++, y += ystep)
                {
                    ...
                    for (int x = r; x < mEye.cols - r; x+=xstep)
                    {
                        ...
                        float response = f.val_inner * sumInner + f.val_outer * sumOuter;
                ...
                [] (const std::pair<float,cv::Point2f>& x, const std::pair<float,cv::Point2f>& y) -> std::pair<float,cv::Point2f>
            {
                if (x.first < y.first)
                    return x;
                else
    ...
    SECTION("Find best region", log)
    {
        ...
        float maxContourArea = cv::contourArea(maxContour);
        ...
        SECTION("Starburst", log)
        {
            ...
            BOOST_FOREACH(const cv::Point2f& centre, centres) {
                tbb::parallel_for(0, params.StarburstPoints, [&] (int i) {
                    float theta = i * 2*PI/params.StarburstPoints;
                    ...
                    while(p.inside(bbPupil))
                    {
                        ...
                        if (val > 0)
                        {
                            ...
                            float dirCheck = dx*cdirx + dy*cdiry;
    ...
    SECTION("Ellipse fitting", log)
    {
        ...
        float w = params.PercentageInliers/100.0;
        ...
        if (edgePoints.size() >= n) // Minimum points for ellipse
        {
            ...
            struct EllipseRansac_out {
                ...
                float bestEllipseGoodness;
                int earlyRejections;
                bool earlyTermination;

                EllipseRansac_out() : bestEllipseGoodness(-std::numeric_limits<float>::infinity()), earlyTermination(false), earlyRejections(0) {}
            };
                    ...
                    for( size_t i=r.begin(); i!=r.end(); ++i )
                    {
                        ...
                        if (params.ImageAwareSupport)
                        {
                            BOOST_FOREACH(cv::Point2f& p, inliers)
                            {
                                ...
                                float edgeStrength = dx*grad.x + dy*grad.y;
