...
cv::RotatedRect pupiltracker::cvx::fitEllipse(const cv::Moments& m)
{
    ...
    float mu20 = m.m20/m.m00 - ret.center.x*ret.center.x;
    float mu02 = m.m02/m.m00 - ret.center.y*ret.center.y;
    float mu11 = m.m11/m.m00 - ret.center.x*ret.center.y;

    float common = std::sqrt(sq(mu20 - mu02) + 4*sq(mu11));
