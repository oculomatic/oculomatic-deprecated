...
namespace cvx
{
    ...
    inline cv::Scalar rgb(float r, float g, float b, float a = 0) {
    ...
    inline cv::Mat resize(const cv::Mat& src, float fx, float fy=0, int interpolation=cv::INTER_LINEAR) {
        if (fy == 0) fy = fx;
        cv::Mat dst;
    ...
    inline cv::Mat GaussianBlur(const cv::Mat& src, cv::Size ksize, float sigmax, float sigmay=0, int borderType=cv::BORDER_DEFAULT) {
        cv::Mat dst;
        cv::GaussianBlur(src, dst, ksize, sigmax, sigmay, borderType);
        return dst;
    }
    inline cv::Mat GaussianBlur(cv::Mat&& src, cv::Size ksize, float sigmax, float sigmay=0, int borderType=cv::BORDER_DEFAULT) {
        cv::GaussianBlur(src, src, ksize, sigmax, sigmay, borderType);
        return src;
    }
    inline cv::Mat GaussianBlur(const cv::Mat& src, float sigmax, float sigmay=0, int borderType=cv::BORDER_DEFAULT) {
        return cvx::GaussianBlur(src, cv::Size(), sigmax, sigmay, borderType);
    }
    inline cv::Mat GaussianBlur(cv::Mat&& src, float sigmax, float sigmay=0, int borderType=cv::BORDER_DEFAULT) {
        return cvx::GaussianBlur(src, cv::Size(), sigmax, sigmay, borderType);
    }

    inline cv::Mat convert(const cv::Mat& src, int rtype, float alpha=1, float beta=0) {
        cv::Mat dst;
